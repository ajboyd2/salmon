...
def anova(model1, model2=None):
    ...
    if model2 is None:
        ...
        raise Exception(
            "Parameters must either be one model or two models where one is a subset of the other.")
...
def _anova_terms(model):
    ...
    global_f_val, global_p_val = _calc_stats(
        full_ssr, full_reg_df, full_sse, full_error_df)
    ...
    for term in terms:
        ...
        reduced_f_val, reduced_p_val = _calc_stats(
            full_ssr - reduced_ssr, term_df, full_sse, full_error_df)
...
def _anova_models(full_model, reduced_model):
    ...
    f_val, p_val = _calc_stats(
        r_sse - f_sse, r_error_df - f_error_df, f_sse, f_error_df)

    indices = ["Full Model", "- Reduced Model",
               "Error"]  # [reduced_label, full_label]
