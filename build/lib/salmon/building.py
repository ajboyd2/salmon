...
class RSquared(Score):
    ...
    def __str__(self):
        return "R^2 ({}adjusted) | {}".format(
            "" if self.adjusted else "un", self._score)
...
def stepwise(full_model, metric_name, forward=False,
             naive=False, data=None, verbose=False):
    ...
    if ex_terms is None or re_term is None:
        raise AssertionError(
            "The full model must be fit prior to undergoing a stepwise procedure.")
    ...
    while len(ex_term_list) > 0:
        ...
        if forward and not naive:
            ...
            leaves = set(
                term for term in ex_term_list if not term.contains(
                    ex_term_list_expression -
                    term))  # Find all terms that do not depend on other terms
            ...
            try:
                if forward:
                    ...
                    potential_model = LinearModel(
                        best_model.given_ex + term, re_term)
                else:
